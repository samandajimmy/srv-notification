...
func NewClientConfig(db *nsql.Database) *ClientConfig {
	...
	updatedNamedColumn := `"updatedAt" = :updatedAt, "metadata" = :metadata, "modifiedBy" = :modifiedBy, "version" = :version, "key" = :key, "value" = :value, "applicationId" = :applicationId, "xid" = :xid`
	...
	return &ClientConfig{
		...
		UpdateByID: db.PrepareNamedFmt(`UPDATE "%s" SET "%s" WHERE id = $1`, tableName, updatedNamedColumn),
